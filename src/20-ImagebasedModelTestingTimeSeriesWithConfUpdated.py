...
x = np.linspace(0, len(confedenceMonthly)-1, len(confedenceMonthly))
z = np.polyfit(x, confedenceMonthly, 3)
p = np.poly1d(z)
y = p(np.arange(0,len(confedenceMonthly)))
plt.plot(x,y,"r--",linewidth=2,label="Confidence")
margin = y-confedenceMonthly
plt.fill_between(x, y, y-margin , color="r", alpha=0.33)
