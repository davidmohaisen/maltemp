...
def getFamiliesSamples(pathMalware, MalwareList,families):
    samples = []
    for fam in families:
        samples.append([])
        for d in MalwareList:
            if d[6] == fam:
                samples[-1].append(pathMalware+d[0])
    return samples


pathMalware = "../Data/Win/FilteredMalware/"
...
samples = getFamiliesSamples(pathMalware, MalwareList,Fam)
print(len(samples))
for s in samples:
    print(len(s))


f = open("../Pickles/VirtualTimeline/VirtualLineSamplesList","wb")
pickle.dump([samples,Fam],f)
