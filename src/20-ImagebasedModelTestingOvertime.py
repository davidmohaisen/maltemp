...
for i in range(len(x_test)):
    ...
    for j in range(len(x_test)):
        ...
        if len(x_test[j]) == 0:
            ...
            x_test_n = np.reshape(x_test[j],(x_test[j].shape[0],x_test[j].shape[1],x_test[j].shape[2],1))
            y_test_n = keras.utils.to_categorical(y_test[j], 2)
            score = model.evaluate(x_test_n, y_test_n, verbose=0)[1]
        matrixToCal[-1].append(score)
        print(i,j,score)

f = open("../Pickles/RetrainingMatrices/Image","wb")
print(matrixToCal)
