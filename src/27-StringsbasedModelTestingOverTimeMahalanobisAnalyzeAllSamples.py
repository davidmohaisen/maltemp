...
for i in range(len(x_test)):
    ...
    for j in range(len(Fam[i])):
        if Fam[i][j] != "SINGLETON" and Fam[i][j] not in TrainFamilies and Fam[i][j] not in currentNewFam:
            AllOut+=1
...
f = open("../Pickles/RetrainingMatrices/StringsMahalanobisAnalysisAllSamples","wb")
