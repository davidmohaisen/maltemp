...
f = open("../Model/Hexdump/BaselineRF_2017.pickle","rb")
clf = pickle.load(f)
...
x_test, y_test =  load_test_data_monthly(pathMalware, pathBenign, MalwareList, BenignList, 2013, 2020)
tprMonthly = []
LabelString = []
indexLabel = []
for i in range(len(x_test)):
...
# T = np.arange(0,len(tprMonthly))
# xnew = np.linspace(T.min(), T.max(), 25)
# power_smooth = make_interp_spline(T, tprMonthly)(xnew)
# plt.plot(xnew,power_smooth, '-ok')
# plt.show()
