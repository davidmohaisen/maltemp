...
def getTrainFamilies(pathMalware, pathBenign, MalwareList, BenignList, year):
    Families = []
...
def load_test_data_monthly_ExceptFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd,FamilyAvoid):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    months = 12
    ...
    for d in MalwareList:
        try:
            if d[2] >= yearStart and d[6] not in FamilyAvoid and d[6] != "SINGLETON":
                f = open(pathMalware+d[0],"rb")
                img = pickle.load(f)
                index = ((d[2]-yearStart)*12)+d[3]-1
                x_test[index].append(img)
...
def load_test_data_weekly_ExceptFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd,FamilyAvoid):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    weeks = 53
...
def load_test_data_monthly_OnlyFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd,FamilyInclude):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    months = 12
    ...
    for d in MalwareList:
        try:
            if d[2] >= yearStart and d[6] in FamilyInclude:
                f = open(pathMalware+d[0],"rb")
                img = pickle.load(f)
                index = ((d[2]-yearStart)*12)+d[3]-1
                x_test[index].append(img)
...
def load_test_data_weekly_OnlyFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd,FamilyInclude):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    weeks = 53
...
def load_test_data_monthly_Singleton(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    months = 12
    ...
    for d in MalwareList:
        try:
            if d[2] >= yearStart and d[6] == "SINGLETON":
                f = open(pathMalware+d[0],"rb")
                img = pickle.load(f)
                index = ((d[2]-yearStart)*12)+d[3]-1
                x_test[index].append(img)
...
def load_test_data_weekly_Singleton(pathMalware, pathBenign, MalwareList, BenignList, yearStart,yearEnd):
    x_test = []
    y_test = []
    years =  yearEnd-yearStart + 1
    weeks = 53
...
# p = np.poly1d(z)
# plt.plot(np.arange(0,len(tprMonthly)),p(np.arange(0,len(tprMonthly))),"k--")
...
# p = np.poly1d(z)
# plt.plot(np.arange(0,len(tprWeekly)),p(np.arange(0,len(tprWeekly))),"k--")
#
# plt.xticks(labels = LabelString, ticks = indexLabel)
# plt.xlabel("Time", fontsize=18)
# plt.ylabel("Performance", fontsize=18)
...
TrainFamilies = getTrainFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart)
x_test, y_test =  load_test_data_monthly_OnlyFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd,TrainFamilies)
tprMonthly = []
LabelString = []
indexLabel = []
for i in range(len(x_test)):
...
plt.plot(np.arange(0,len(tprMonthly)), tprMonthly, '-ok',label='Seen Families');
z = np.polyfit(np.arange(0,len(tprMonthly)), tprMonthly, 1)
p = np.poly1d(z)
...
# x_test, y_test =  load_test_data_weekly_OnlyFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd,TrainFamilies)
# tprWeekly = []
# LabelString = []
# indexLabel = []
# for i in range(len(x_test)):
...
TrainFamilies = getTrainFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart)
x_test, y_test =  load_test_data_monthly_ExceptFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd,TrainFamilies)
tprMonthly = []
LabelString = []
indexLabel = []
for i in range(len(x_test)):
...
# plt.plot(np.arange(0,len(tprMonthly)), tprMonthly, '-ok');
plt.plot(np.arange(0,len(tprMonthly)), tprMonthly, '-og',label='New Families');
z = np.polyfit(np.arange(0,len(tprMonthly)), tprMonthly, 1)
p = np.poly1d(z)
# plt.plot(np.arange(0,len(tprMonthly)),p(np.arange(0,len(tprMonthly))),"k--")
plt.plot(np.arange(0,len(tprMonthly)),p(np.arange(0,len(tprMonthly))),"g--",linewidth=2)
plt.xticks(labels = LabelString, ticks = indexLabel)
plt.xlabel("Time", fontsize=18)
plt.ylabel("Performance", fontsize=18)
plt.xticks(fontsize= 12)
...
# x_test, y_test =  load_test_data_weekly_ExceptFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd,TrainFamilies)
# tprWeekly = []
# LabelString = []
# indexLabel = []
# for i in range(len(x_test)):
...
TrainFamilies = getTrainFamilies(pathMalware, pathBenign, MalwareList, BenignList, yearStart)
x_test, y_test =  load_test_data_monthly_Singleton(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd)
tprMonthly = []
LabelString = []
indexLabel = []
for i in range(len(x_test)):
...
# plt.plot(np.arange(0,len(tprMonthly)), tprMonthly, '-ok');
plt.plot(np.arange(0,len(tprMonthly)), tprMonthly, '-or',label='Singleton');
z = np.polyfit(np.arange(0,len(tprMonthly)), tprMonthly, 1)
p = np.poly1d(z)
# plt.plot(np.arange(0,len(tprMonthly)),p(np.arange(0,len(tprMonthly))),"k--")
plt.plot(np.arange(0,len(tprMonthly)),p(np.arange(0,len(tprMonthly))),"r--",linewidth=2)
plt.xticks(labels = LabelString, ticks = indexLabel)
plt.xlabel("Time", fontsize=18)
plt.ylabel("Performance", fontsize=18)
plt.xticks(fontsize= 12)
...
# x_test, y_test =  load_test_data_weekly_Singleton(pathMalware, pathBenign, MalwareList, BenignList, yearStart, yearEnd)
# tprWeekly = []
# LabelString = []
# indexLabel = []
# for i in range(len(x_test)):
