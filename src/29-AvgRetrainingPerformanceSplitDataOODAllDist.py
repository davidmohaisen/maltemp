...
for name in matricesNames:
    ...
    for i in range(len(d)):
        ...
        if (pointerI < len(When) and i == When[pointerI]):
            pointerI += 1
            currentI = i
            counter = 0
            TimesRetrain += 1
    ...
    results.append(str(TimesRetrain))

    file = open(path+name+"OODManhattan","rb")
    _,When = pickle.load(file)
    file = open(path+name,"rb")
    d = pickle.load(file)
    currentI = 0
    ...
    file = open(path+name+"OODCosine","rb")
    _,When = pickle.load(file)
    file = open(path+name,"rb")
    d = pickle.load(file)
    currentI = 0
    ...
    file = open(path+name+"OODHamming","rb")
    _,When = pickle.load(file)
    file = open(path+name,"rb")
    d = pickle.load(file)
    currentI = 0
