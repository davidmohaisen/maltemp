...
for i in range(len(x_test_in)):
    ...
    f = open("../Model/Relocs/Model_Week_"+str(i),"rb")
    clf = pickle.load(f)
    ...
    for j in range(len(x_test_in)):
        ...
        if len(x_test_in[j]) == 0:
            ...
            x_test_n = vectorize(x_test_in[j],dic)
        ...
        if len(x_test_out[j]) == 0:
            ...
            x_test_n = vectorize(x_test_out[j],dic)
        ...
        if len(x_test_sing[j]) == 0:
            ...
            x_test_n = vectorize(x_test_sing[j],dic)
...
f = open("../Pickles/RetrainingMatrices/RelocsSplitData","wb")
