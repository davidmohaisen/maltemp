...
json_file = open("../Model/Image/Model_Week_0.json",'r')
loaded_model_json = json_file.read()
json_file.close()
model = model_from_json(loaded_model_json)
model.load_weights("../Model/Image/Model_Week_0.h5")
...
for i in range(len(x_test)):
    ...
    if len(x_test[i]) == 0:
        continue
    x_test_n = np.reshape(x_test[i],(x_test[i].shape[0],x_test[i].shape[1],x_test[i].shape[2],1))
    y_test_n = keras.utils.to_categorical(y_test[i], 2)

    preds = model.predict(x_test_n)
    Performance.append(model.evaluate(x_test_n, y_test_n, verbose=0)[1])
    average = 0
    lowest = 1
...
# f = open("../Pickles/RetrainingMatrices/ImageConfMinimum","wb")
...
f = open("../Pickles/RetrainingMatrices/ImageConfAVG","wb")
