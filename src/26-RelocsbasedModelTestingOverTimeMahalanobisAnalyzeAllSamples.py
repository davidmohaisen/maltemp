...
f = open("../Pickles/RetrainingMatrices/Relocs","rb")
...
for i in range(len(x_test)):
    ...
    for j in range(len(Fam[i])):
        if Fam[i][j] != "SINGLETON" and Fam[i][j] not in TrainFamilies:
            AllOut+=1
            uniqueFamilies.append(Fam[i][j])
    for j in range(len(Fam[i])):
...
f = open("../Pickles/RetrainingMatrices/RelocsMahalanobisAnalysisAllSamples","wb")
...
uniqueFamilies = list(set(uniqueFamilies))
print(AllOut,len(uniqueFamilies))
