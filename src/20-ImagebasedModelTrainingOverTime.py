...
for i in range(len(x_test)):
    if i != 0:
        ...
        if len(x_test[i-1])!= 0:
            x_train = np.concatenate((x_train,x_test[i-1]),axis = 0)
            y_train = np.concatenate((y_train,y_test[i-1]),axis = 0)

    x_train_n = np.reshape(x_train,(x_train.shape[0],x_train.shape[1],x_train.shape[2],1))
    y_train_n = keras.utils.to_categorical(y_train, 2)
    ...
    with open("../Model/Image/Model_Week_"+str(i)+".json", "w") as json_file:
        json_file.write(model_json)
    model.save_weights("../Model/Image/Model_Week_"+str(i)+".h5")
